...
//implemntating comparable pokemon
   ...
   **/
   public Pokemon(String species, String name,  
      ...
      //this triggers hidden fields from the 
      //base code from A3 provided by the professor
  ...
  */
   ...
   public void setName(String newName) throws PokemonException {
      ...
      if (newName.matches("")) { 
      //Throw the exception if string for name is empty
         PokemonException ex = new PokemonException();
         ex.setMessage("Blank entry! Your Pokemon has no name!");
         throw ex;
      }
      
      else { //not empty 
      
         this.name = newName; //name is assigned
      }
   }
   ...
   */  
   public String getSpecialAttack() {
    ...
    /**
    * Method for comparison of Pokemon.
    * @param p for Pokemon.
    * @return r for result.
    */
   ...
   public int compareTo(Pokemon p) {
      /**
       * Pokemon 1 declaration.
       */
      /** Variables. */
      
      int num1 = this.number;
      /** Set the name for first Pokemon. */
      String name1 = this.name;
      /** Set the HP for first Pokemon. */
      int hP1 = this.hP;
      /** Set the CP for first Pokemon. */
      int cP1 = this.cP;
      /** Set the fastAttack for first Pokemon. */
      String fastAttack1 = this.fastAttack;
      /** Set the specialAttack for first Pokemon. */
      String specialAttack1 = this.specialAttack;
   
      /**
       * Pokemon 2 declaration.
       */
      /** Variables. */
      
      int num2 = p.getNumber();
      /** Set the name for second Pokemon. */
      String name2 = p.getName();
      /** Set the HP for second Pokemon. */
      int hP2 = p.getHP();
      /** Set the CP for second Pokemon. */
      int cP2 = p.getCP();
      /** Set the fastAttack for second Pokemon. */
      String fastAttack2 = p.getFastAttack();
      /** Set the specialAttack for second Pokemon. */
      String specialAttack2 = p.getSpecialAttack(); 
      //all from original pokemon.java
      ...
      if (num1 != num2) { 
         ...
         //Compare the names of the two Pokemon
         r = name1.toUpperCase().compareTo(name2.toUpperCase());
         if (r == 0) { 
            if (hP1 != hP2) { //comparison of hp
               r = hP1 - hP2; 
            }
            else {
               if (cP1 != cP2) { //comparison of cp
                  r = cP1 - cP2; 
               }
               else {
                  //comparison of fast attacks
                  r = fastAttack1.compareTo(fastAttack2);
                  if (r == 0) { //Check if result is equal to 0.
                     //comparison of special attacks
